...
impl MidiEditor {
        ...
        for event in events.iter_mut() {
            if let MidiMessage::NoteOn { .. } = event.message {
                ...
                event.time = event.time
                    + (target_time - event.time) * self.quantize_settings.strength as f64;
            }
        }
    }

    pub fn edit_velocities(
        &self,
        ...
        for (index, event) in selected_events.into_iter().enumerate() {
            if let MidiMessage::NoteOn { velocity, .. } = &mut event.message {
                let new_velocity = match self.velocity_settings.mode {
                    VelocityEditMode::Set => self.velocity_settings.amount as u8,
                    VelocityEditMode::Add => {
                        (*velocity as f32 + self.velocity_settings.amount).clamp(1.0, 127.0) as u8
                    }
                    VelocityEditMode::Scale => (*velocity as f32 * self.velocity_settings.amount
                        / 100.0)
                        .clamp(1.0, 127.0) as u8,
                    VelocityEditMode::Compress => {
                        let center = 64.0;
                        let diff = *velocity as f32 - center;
                        (center + diff * (1.0 - self.velocity_settings.amount / 100.0))
                            .clamp(1.0, 127.0) as u8
                    }
                ...
                let final_velocity = if self.velocity_settings.randomize > 0.0 {
                    let random_offset = (rand::random::<f32>() - 0.5)
                        * 2.0
                        * self.velocity_settings.randomize
                        * 20.0; // Max ±20 velocity units
    ...
    pub fn interpolate_controller_values(
        ...
        if let Some(lane) = self.controller_lanes.get_mut(&controller) {
            ...
            lane.events
                .retain(|e| e.time < start_time || e.time > end_time);
            ...
            for i in 0..=steps {
                ...
                let value =
                    (start_value as f64 + (end_value as f64 - start_value as f64) * progress) as u8;
